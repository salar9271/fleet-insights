...
def main():
    ...
    # Small metadata file the API reads at startup instead of re-fitting the
    # whole clustering pipeline just to get test_ari and feature_cols -- see
    # src/api.py. Keeping this next to the joblib artifacts it describes
    # means both are always regenerated by the same run of this script.
    metadata = {
        "feature_cols": results["feature_cols"],
        "best_k": results["best_k"],
        "n_train_windows": int(len(results["train_labels"])),
        "n_test_windows": int(len(results["test_labels"])),
        "train_ari": float(results["train_ari"]),
        "test_ari": float(results["test_ari"]),
    }
